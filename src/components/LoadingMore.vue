...
const props = withDefaults(defineProps<LoadingMoreProps>(), {
  status: 'more',
  showIcon: true,
  iconType: 'auto',
  color: '#777777',
...
interface LoadingMoreContentText {
...
interface LoadingMoreProps {
  ...
  iconType?: string
  color?: string
  contentText?: LoadingMoreContentText
}

const { status, showIcon, iconType, contentText } = toRefs(props)
</script>

<template>
  <view class="loading-more">
    <view
      v-if="(iconType === 'circle' || iconType === 'auto' && platform === 'android') && status === 'more' && showIcon"
      class="uni-load-more__img"
    >
      <image
        class="uni-load-more__img--android"
    ...
    <view v-else-if="status === 'more' && showIcon" class="uni-load-more__img">
      <image
        class="uni-load-more__img--ios"
